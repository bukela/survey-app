    ...
    if ($('.button__purple').length) // use this if you are using id to check
        {
            ...
            var buttonPress = function buttonPress(elem, event) {
            ...
            // disable enter key if not textarea
            $('form').keydown(function (event) {
                var selectedElem = $(event.target).attr('id');
                var allTextareas = $(this).find('textarea');
                if (allTextareas.length > 0) {
                    var textareaIdList = [];
                    $.each(allTextareas, function (i, v) {
                        textareaIdList.push($(v).attr('id'));
                    });
                    if (event.keyCode == 13 && !textareaIdList.includes(selectedElem)) {
...
(function ($) {
	...
	is_click = false,
	    ...
	    is_fx_mobile = function is_fx_mobile() {
		if (picker && pickers[picker.id].fx && !pickers[picker.id].fxmobile) {
			if ($(window).width() < 480) picker.element.removeClass('picker-fxs');else picker.element.addClass('picker-fxs');
		}
	},
	    is_jumpable = function is_jumpable() {
		if (pickers[picker.id].jump >= pickers[picker.id].key.y.max - pickers[picker.id].key.y.min) return false;else return true;
	},
	...
	get_current = function get_current(k) {
	    ...
	    get_closest_jumped = function get_closest_jumped(int, arr) {
		var c = arr[0];
	    ...
	    get_clear = function get_clear(k, n) {
		var key_values = pickers[picker.id].key[k];
		if (n > key_values.max) return get_clear(k, n - key_values.max + (key_values.min - 1));else if (n < key_values.min) return get_clear(k, n + 1 + (key_values.max - key_values.min));else return n;
	},
	    get_days_array = function get_days_array() {
		if (i18n[pickers[picker.id].lang].gregorian) return [1, 2, 3, 4, 5, 6, 0];else return [0, 1, 2, 3, 4, 5, 6];
	},
	    get_ul = function get_ul(k) {
		return get_picker_els('ul.pick[data-k="' + k + '"]');
	},
	    get_eq = function get_eq(k, d) {
		ul = get_ul(k);
		var o = [];
		...
		if (d == 'last') return o[o.length - 1];else return o[0];
	},
	    get_picker_els = function get_picker_els(el) {
		if (picker) return picker.element.find(el);
	},
	    get_unix = function get_unix(d) {
	...
	picker_large_onoff = function picker_large_onoff() {
		if (pickers[picker.id].large) {
			picker.element.toggleClass('picker-lg');
			picker_render_calendar();
		}
	},
	    picker_translate_onoff = function picker_translate_onoff() {
		get_picker_els('ul.pick.pick-l').toggleClass('visible');
	},
	    picker_offset = function picker_offset() {
	    ...
	    picker_translate = function picker_translate(v) {
		pickers[picker.id].lang = Object.keys(i18n)[v];
		picker_set_lang();
		picker_set();
	},
	    picker_set_lang = function picker_set_lang() {
	    ...
	    picker_show = function picker_show() {
		picker.element.addClass('picker-focus');
	},
	    picker_hide = function picker_hide() {
		if (!is_locked()) {
			picker.element.removeClass('picker-focus');
			if (picker.element.hasClass('picker-modal')) $('.picker-modal-overlay').addClass('tohide');
			picker = null;
		}
		picker_ctrl = false;
	},
	    picker_render_ul = function picker_render_ul(k) {
		var ul = get_ul(k),
		    key_values = pickers[picker.id].key[k];
		...
		for (i = key_values.min; i <= key_values.max; i++) {
			...
			if (k == 'm') html = i18n[pickers[picker.id].lang].months.short[i - 1];
			if (k == 'l') html = i18n[Object.keys(i18n)[i]].name;
	    ...
	    picker_render_calendar = function picker_render_calendar() {
		...
		if (w.find('li.pick-n').length < 42) {
			...
			for (var i = 1; i <= e; i++) {
				w.find('li').eq(index).html(i).addClass('pick-a pick-n pick-h');
				index++;
			}
		}
		if (pickers[picker.id].lock) {
		...
		if (pickers[picker.id].disabledays) {
			$.each(pickers[picker.id].disabledays, function (i, v) {
				if (v && is_date(v)) {
					...
					if (d.getMonth() + 1 == get_current('m') && d.getFullYear() == get_current('y')) get_picker_els('.pick-lg .pick-lg-b li.pick-v[data-value="' + d.getDate() + '"]').addClass('pick-lk');
				}
			});
		}
	    ...
	    picker_fills = function picker_fills() {
	    ...
	    picker_set = function picker_set() {
		if (picker.element.hasClass('picker-lg')) picker_render_calendar();
		picker_fills();
		input_change_value();
	},
	...
	picker_ul_transition = function picker_ul_transition(k, i) {
	    ...
	    picker_values_increase = function picker_values_increase(k, v) {

		var key_values = pickers[picker.id].key[k];

		if (v > key_values.max) {
			if (k == 'd') picker_ul_turn('m', 'right');
			if (k == 'm') picker_ul_turn('y', 'right');
			v = key_values.min;
		}
		if (v < key_values.min) {
			if (k == 'd') picker_ul_turn('m', 'left');
			if (k == 'm') picker_ul_turn('y', 'left');
			v = key_values.max;
		}
		pickers[picker.id].key[k].current = v;
		picker_ul_transition(k, v);
	},
	    picker_ul_turn = function picker_ul_turn(k, d) {
		var v = get_current(k);
		if (d == 'right') v++;else v--;
		picker_values_increase(k, v);
	},
	    picker_alrt = function picker_alrt() {
	...
	input_fill = function input_fill(n) {
		return n < 10 ? '0' + n : n;
	},
	    input_ordinal_suffix = function input_ordinal_suffix(n) {
		var s = ["th", "st", "nd", "rd"],
		    v = n % 100;
		return n + (s[(v - 20) % 10] || s[v] || s[0]);
	},
	    input_change_value = function input_change_value() {
		...
		if (!is_locked() && picker_ctrl) {
			...
			var d = get_current('d'),
			    ...
			    str = pickers[picker.id].format.replace(/\b(d)\b/g, input_fill(d)).replace(/\b(m)\b/g, input_fill(m)).replace(/\b(S)\b/g, input_ordinal_suffix(d)) //new
			.replace(/\b(Y)\b/g, y).replace(/\b(U)\b/g, get_unix(get_current_full())) //new
			.replace(/\b(D)\b/g, i18n[pickers[picker.id].lang].weekdays.short[get_day]).replace(/\b(l)\b/g, i18n[pickers[picker.id].lang].weekdays.full[get_day]).replace(/\b(F)\b/g, i18n[pickers[picker.id].lang].months.full[m - 1]).replace(/\b(M)\b/g, i18n[pickers[picker.id].lang].months.short[m - 1]).replace(/\b(n)\b/g, m).replace(/\b(j)\b/g, d);
	...
	.on(csse.a, picker_node_el + '.picker-rmbl', function () {
		if (picker.element.hasClass('picker-rmbl')) $(this).removeClass('picker-rmbl');
	})
	...
	.on(ui_event.i, picker_node_el + ' .pick-arw.pick-arw-s2', function (e) {
		...
		if ($(this).hasClass('pick-arw-r')) i = get_current('y') + jump;else i = get_current('y') - jump;
	...
	.on(ui_event.i, picker_node_el + ' .pick-arw.pick-arw-s1', function (e) {
		...
		if ($(this).hasClass('pick-arw-r')) picker_ul_turn(k, 'right');else picker_ul_turn(k, 'left');
	...
	.on(ui_event.m, function (e) {
		...
		if (pick_dragged) {
			...
			if (int != pickers[picker.id].key[k].current) picker_values_increase(k, int);
	...
	.on(ui_event.e, function (e) {
		if (pick_dragged) pick_dragged = null, pick_drag_offset = null, pick_drag_temp = null;
		if (picker) picker_set();
	})
	...
	$.fn.dateDropper = function (options) {
		return $(this).each(function () {
			if ($(this).is('input') && !$(this).hasClass('picker-input')) {
				...
				var picker_default_date = input.data('default-date') && is_date(input.data('default-date')) ? input.data('default-date') : null,
				...
				if (picker_disabled_days) {
					$.each(picker_disabled_days, function (index, value) {
						if (value && is_date(value)) picker_disabled_days[index] = get_unix(value);
					});
				}
				...
				if (picker_default_date) {
					...
					if (pickers[id].key.y.today > pickers[id].key.y.max) pickers[id].key.y.max = pickers[id].key.y.today;
					if (pickers[id].key.y.today < pickers[id].key.y.min) pickers[id].key.y.min = pickers[id].key.y.today;
				}
				...
				if (picker_format == 'Y' || picker_format == 'm') {
					...
					if (picker_format == 'Y') get_picker_els('.pick-m,.pick-btn-lng').hide();
					if (picker_format == 'm') get_picker_els('.pick-y').hide();
				}
			...
			if (picker.element.hasClass('picker-modal')) $('body').append('<div class="picker-modal-overlay"></div>');
		});
	};
})(jQuery);
